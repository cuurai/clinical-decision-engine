...
import { createClinicalRule, deleteClinicalRule, getClinicalRule, listClinicalRules, updateClinicalRule } from "@cuur-cde/core/knowledge-evidence/handlers/index.js";
import type { ClinicalRuleInput, ClinicalRuleUpdate } from "@cuur-cde/core/knowledge-evidence/types/index.js";
...
export async function clinicalRulesRoutes(
  ...
  fastify.patch("/clinical-rules/:id", async (request, reply) => {
    const orgId = extractOrgId(request);
        const id = (request.params as any).id;
        const result = await updateClinicalRule(deps.clinicalRuleRepo, orgId, id, request.body as ClinicalRuleUpdate);
    return reply.code(200).send(result);
  });
