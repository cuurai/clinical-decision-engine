/**
 * DAO adapter for GuidelineEvidenceCitationRepository
 *
...
export class DaoGuidelineEvidenceCitationRepository implements GuidelineEvidenceCitationRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.guidelineEvidenceCitation.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<GuidelineEvidenceCitation | null> {
    try {
      const record = await this.dao.guidelineEvidenceCitation.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<GuidelineEvidenceCitation | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("GuidelineEvidenceCitation", id);
    }
    return result;
  }
  private toDomain(model: any): GuidelineEvidenceCitation {
    return {
      ...model,
