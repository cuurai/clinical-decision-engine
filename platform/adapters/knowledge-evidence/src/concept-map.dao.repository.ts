...
import type { ConceptMapRepository } from "@cuur-cde/core/knowledge-evidence/repositories/index.js";
import type { UpdateConceptMapRequest } from "@cuur-cde/core/knowledge-evidence/types/index.js";
import type {
  ConceptMapInput,
  ConceptMapUpdate,
  ConceptMap,
...
export class DaoConceptMapRepository implements ConceptMapRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<ConceptMap>> {
    try {
    ...
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const item of items) {
          const record = await tx.conceptMapInput.create({
            data: {
              ...item,
              orgId,
            },
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: ConceptMapUpdate }>
  ): Promise<ConceptMap[]> {
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await tx.conceptMapInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
