/**
 * DAO adapter for OntologyTermParentRepository
 *
...
export class DaoOntologyTermParentRepository implements OntologyTermParentRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.ontologyTermParent.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<OntologyTermParent | null> {
    try {
      const record = await this.dao.ontologyTermParent.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<OntologyTermParent | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("OntologyTermParent", id);
    }
    return result;
  }
  private toDomain(model: any): OntologyTermParent {
    return {
      ...model,
