/**
 * DAO adapter for GuidelineSectionRepository
 *
...
export class DaoGuidelineSectionRepository implements GuidelineSectionRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.guidelineSection.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<GuidelineSection | null> {
    try {
      const record = await this.dao.guidelineSection.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<GuidelineSection | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("GuidelineSection", id);
    }
    return result;
  }
  private toDomain(model: any): GuidelineSection {
    return {
      ...model,
