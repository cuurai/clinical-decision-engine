/**
 * DAO adapter for OntologyTermChildRepository
 *
...
export class DaoOntologyTermChildRepository implements OntologyTermChildRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.ontologyTermChild.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<OntologyTermChild | null> {
    try {
      const record = await this.dao.ontologyTermChild.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<OntologyTermChild | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("OntologyTermChild", id);
    }
    return result;
  }
  private toDomain(model: any): OntologyTermChild {
    return {
      ...model,
