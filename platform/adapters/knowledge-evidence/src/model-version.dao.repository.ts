...
import type { ModelVersionRepository } from "@cuur-cde/core/knowledge-evidence/repositories/index.js";
import type { UpdateModelVersionRequest } from "@cuur-cde/core/knowledge-evidence/types/index.js";
import type {
  ModelVersionInput,
  ModelVersionUpdate,
  ModelVersion,
    ...
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const item of items) {
          const record = await tx.modelVersionInput.create({
            data: {
              ...item,
              orgId,
            },
    ...
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await tx.modelVersionInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
