/**
 * DAO adapter for OrderSetTemplateItemRepository
 *
...
export class DaoOrderSetTemplateItemRepository implements OrderSetTemplateItemRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.orderSetTemplateItem.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<OrderSetTemplateItem | null> {
    try {
      const record = await this.dao.orderSetTemplateItem.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<OrderSetTemplateItem | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("OrderSetTemplateItem", id);
    }
    return result;
  }
  private toDomain(model: any): OrderSetTemplateItem {
    return {
      ...model,
