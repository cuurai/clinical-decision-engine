/**
 * DAO adapter for ClinicalRuleTestRepository
 *
...
export class DaoClinicalRuleTestRepository implements ClinicalRuleTestRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.clinicalRuleTest.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ClinicalRuleTest | null> {
    try {
      const record = await this.dao.clinicalRuleTest.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ClinicalRuleTest | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ClinicalRuleTest", id);
    }
    return result;
  }
  private toDomain(model: any): ClinicalRuleTest {
    return {
      ...model,
