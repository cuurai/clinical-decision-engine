/**
 * DAO adapter for ModelDefinitionVersionRepository
 *
...
export class DaoModelDefinitionVersionRepository implements ModelDefinitionVersionRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.modelDefinitionVersion.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ModelDefinitionVersion | null> {
    try {
      const record = await this.dao.modelDefinitionVersion.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ModelDefinitionVersion | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ModelDefinitionVersion", id);
    }
    return result;
  }
  private toDomain(model: any): ModelDefinitionVersion {
    return {
      ...model,
