...
export class DaoKnowledgePackageRepository implements KnowledgePackageRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<KnowledgePackage>> {
    try {
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const item of items) {
          const record = await txClient.knowledgePackageInput.create({
            data: {
              ...item,
              orgId,
            },
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: KnowledgePackageUpdate }>
  ): Promise<KnowledgePackage[]> {
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await txClient.knowledgePackageInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
