...
import type { UpdateQuestionnaireTemplateRequest } from "@cuur-cde/core/knowledge-evidence";
import type {
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const item of items) {
          const record = await txClient.questionnaireTemplateInput.create({
            data: {
              ...item,
              orgId,
            },
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await txClient.questionnaireTemplateInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
