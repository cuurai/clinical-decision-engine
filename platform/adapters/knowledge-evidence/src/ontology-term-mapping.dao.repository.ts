/**
 * DAO adapter for OntologyTermMappingRepository
 *
...
export class DaoOntologyTermMappingRepository implements OntologyTermMappingRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.ontologyTermMapping.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<OntologyTermMapping | null> {
    try {
      const record = await this.dao.ontologyTermMapping.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<OntologyTermMapping | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("OntologyTermMapping", id);
    }
    return result;
  }
  private toDomain(model: any): OntologyTermMapping {
    return {
      ...model,
