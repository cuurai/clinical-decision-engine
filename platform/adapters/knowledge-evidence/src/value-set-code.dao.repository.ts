/**
 * DAO adapter for ValueSetCodeRepository
 *
...
export class DaoValueSetCodeRepository implements ValueSetCodeRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.valueSetCode.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ValueSetCode | null> {
    try {
      const record = await this.dao.valueSetCode.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ValueSetCode | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ValueSetCode", id);
    }
    return result;
  }
  private toDomain(model: any): ValueSetCode {
    return {
      ...model,
