/**
 * DAO adapter for OntologyTermRepository
 *
...
export class DaoOntologyTermRepository implements OntologyTermRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.ontologyTerm.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<OntologyTerm | null> {
    try {
      const record = await this.dao.ontologyTerm.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<OntologyTerm | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("OntologyTerm", id);
    }
    return result;
  }
  private toDomain(model: any): OntologyTerm {
    return {
      ...model,
