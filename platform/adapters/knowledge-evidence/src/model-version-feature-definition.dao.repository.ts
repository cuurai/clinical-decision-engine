/**
 * DAO adapter for ModelVersionFeatureDefinitionRepository
 *
...
export class DaoModelVersionFeatureDefinitionRepository implements ModelVersionFeatureDefinitionRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.modelVersionFeatureDefinition.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ModelVersionFeatureDefinition | null> {
    try {
      const record = await this.dao.modelVersionFeatureDefinition.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ModelVersionFeatureDefinition | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ModelVersionFeatureDefinition", id);
    }
    return result;
  }
  private toDomain(model: any): ModelVersionFeatureDefinition {
    return {
      ...model,
    ...
    } as ModelVersionFeatureDefinition;
  }
}
