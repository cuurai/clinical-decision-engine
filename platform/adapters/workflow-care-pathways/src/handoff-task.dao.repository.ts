/**
 * DAO adapter for HandoffTaskRepository
 *
...
export class DaoHandoffTaskRepository implements HandoffTaskRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.handoffTask.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<HandoffTask | null> {
    try {
      const record = await this.dao.handoffTask.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<HandoffTask | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("HandoffTask", id);
    }
    return result;
  }
  private toDomain(model: any): HandoffTask {
    return {
      ...model,
