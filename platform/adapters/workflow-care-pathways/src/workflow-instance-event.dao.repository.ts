/**
 * DAO adapter for WorkflowInstanceEventRepository
 *
...
export class DaoWorkflowInstanceEventRepository implements WorkflowInstanceEventRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.workflowInstanceEvent.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<WorkflowInstanceEvent | null> {
    try {
      const record = await this.dao.workflowInstanceEvent.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<WorkflowInstanceEvent | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("WorkflowInstanceEvent", id);
    }
    return result;
  }
  private toDomain(model: any): WorkflowInstanceEvent {
    return {
      ...model,
