/**
 * DAO adapter for EpisodeOfCareEncounterRepository
 *
...
export class DaoEpisodeOfCareEncounterRepository implements EpisodeOfCareEncounterRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.episodeOfCareEncounter.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<EpisodeOfCareEncounter | null> {
    try {
      const record = await this.dao.episodeOfCareEncounter.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<EpisodeOfCareEncounter | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("EpisodeOfCareEncounter", id);
    }
    return result;
  }
  private toDomain(model: any): EpisodeOfCareEncounter {
    return {
      ...model,
