/**
 * DAO adapter for CarePathwayTemplateOrderSetTemplateRepository
 *
...
export class DaoCarePathwayTemplateOrderSetTemplateRepository implements CarePathwayTemplateOrderSetTemplateRepository {
  ...
  async get(orgId: OrgId, id: string): Promise<CarePathwayTemplateOrderSetTemplate | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("CarePathwayTemplateOrderSetTemplate", id);
    }
    return result;
  }
  private toDomain(model: any): CarePathwayTemplateOrderSetTemplate {
    return {
      ...model,
    ...
    } as CarePathwayTemplateOrderSetTemplate;
  }
}
