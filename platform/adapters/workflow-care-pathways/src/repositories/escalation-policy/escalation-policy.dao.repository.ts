/**
 * DAO adapter for EscalationPolicyRepository
 *
...
export class DaoEscalationPolicyRepository implements EscalationPolicyRepository {
  ...
  async get(orgId: OrgId, id: string): Promise<EscalationPolicy | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("EscalationPolicy", id);
    }
    return result;
  }
  async create(orgId: OrgId, data: EscalationPolicy): Promise<EscalationPolicy> {
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const item of items) {
          const record = await txClient.escalationPolicyInput.create({
            data: {
              ...item,
              orgId,
            },
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await txClient.escalationPolicyInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
