...
export class DaoChecklistTemplateRepository implements ChecklistTemplateRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<ChecklistTemplate>> {
    try {
  ...
  async createMany(
    orgId: OrgId,
    items: Array<ChecklistTemplateInput>
  ): Promise<ChecklistTemplate[]> {
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const item of items) {
          const record = await txClient.checklistTemplateInput.create({
            data: {
              ...item,
              orgId,
            },
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: ChecklistTemplateUpdate }>
  ): Promise<ChecklistTemplate[]> {
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await txClient.checklistTemplateInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
