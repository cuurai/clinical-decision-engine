/**
 * DAO adapter for RoutingRuleRepository
 *
...
export class DaoRoutingRuleRepository implements RoutingRuleRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<RoutingRule>> {
    try {
  ...
  async get(orgId: OrgId, id: string): Promise<RoutingRule | null> {
  ...
  async create(orgId: OrgId, data: RoutingRule): Promise<RoutingRule> {
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const item of items) {
          const record = await txClient.routingRuleInput.create({
            data: {
              ...item,
              orgId,
            },
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: RoutingRuleUpdate }>
  ): Promise<RoutingRule[]> {
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await txClient.routingRuleInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
  ...
  private toDomain(model: any): RoutingRule {
    return {
      ...model,
