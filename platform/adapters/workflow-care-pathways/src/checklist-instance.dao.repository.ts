...
import type { ChecklistInstanceRepository } from "@cuur-cde/core/workflow-care-pathways/repositories/index.js";
import type {
    ...
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const item of items) {
          const record = await tx.checklistInstanceInput.create({
            data: {
              ...item,
              orgId,
            },
    ...
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await tx.checklistInstanceInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
