/**
 * DAO adapter for CarePlanGoalRepository
 *
...
export class DaoCarePlanGoalRepository implements CarePlanGoalRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.carePlanGoal.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<CarePlanGoal | null> {
    try {
      const record = await this.dao.carePlanGoal.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<CarePlanGoal | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("CarePlanGoal", id);
    }
    return result;
  }
  private toDomain(model: any): CarePlanGoal {
    return {
      ...model,
