/**
 * DAO adapter for CarePlanTaskRepository
 *
...
export class DaoCarePlanTaskRepository implements CarePlanTaskRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.carePlanTask.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<CarePlanTask | null> {
    try {
      const record = await this.dao.carePlanTask.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<CarePlanTask | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("CarePlanTask", id);
    }
    return result;
  }
  private toDomain(model: any): CarePlanTask {
    return {
      ...model,
