/**
 * DAO adapter for AlertExplanationRepository
 *
...
export class DaoAlertExplanationRepository implements AlertExplanationRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.alertExplanation.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<AlertExplanation | null> {
    try {
      const record = await this.dao.alertExplanation.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<AlertExplanation | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("AlertExplanation", id);
    }
    return result;
  }
  private toDomain(model: any): AlertExplanation {
    return {
      ...model,
