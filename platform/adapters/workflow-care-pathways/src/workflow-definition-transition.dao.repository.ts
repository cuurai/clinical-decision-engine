/**
 * DAO adapter for WorkflowDefinitionTransitionRepository
 *
...
export class DaoWorkflowDefinitionTransitionRepository implements WorkflowDefinitionTransitionRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.workflowDefinitionTransition.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<WorkflowDefinitionTransition | null> {
    try {
      const record = await this.dao.workflowDefinitionTransition.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<WorkflowDefinitionTransition | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("WorkflowDefinitionTransition", id);
    }
    return result;
  }
  private toDomain(model: any): WorkflowDefinitionTransition {
    return {
      ...model,
    ...
    } as WorkflowDefinitionTransition;
  }
}
