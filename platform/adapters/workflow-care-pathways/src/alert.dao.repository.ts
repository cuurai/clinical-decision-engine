/**
 * DAO adapter for AlertRepository
 *
...
import type { AlertRepository } from "@cuur-cde/core/workflow-care-pathways/repositories/index.js";
import type {
  AlertInput,
  AlertUpdate,
  UpdateAlertRequest,
...
export class DaoAlertRepository implements AlertRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<Alert>> {
    try {
  ...
  async get(orgId: OrgId, id: string): Promise<Alert | null> {
  ...
  async create(orgId: OrgId, data: Alert): Promise<Alert> {
    // Note: Repository interface expects Alert, but we only use input fields
  ...
  async createMany(orgId: OrgId, items: Array<AlertInput>): Promise<Alert[]> {
    try {
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: AlertUpdate }>
  ): Promise<Alert[]> {
    try {
  ...
  private toDomain(model: any): Alert {
    return {
      ...model,
