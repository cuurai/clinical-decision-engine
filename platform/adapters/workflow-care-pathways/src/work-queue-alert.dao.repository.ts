/**
 * DAO adapter for WorkQueueAlertRepository
 *
...
export class DaoWorkQueueAlertRepository implements WorkQueueAlertRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.workQueueAlert.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<WorkQueueAlert | null> {
    try {
      const record = await this.dao.workQueueAlert.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<WorkQueueAlert | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("WorkQueueAlert", id);
    }
    return result;
  }
  private toDomain(model: any): WorkQueueAlert {
    return {
      ...model,
