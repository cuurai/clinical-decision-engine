/**
 * DAO adapter for CarePlanRepository
 *
...
import type { CarePlanRepository } from "@cuur-cde/core/workflow-care-pathways/repositories/index.js";
import type {
  CarePlanInput,
  CarePlanUpdate,
...
export class DaoCarePlanRepository implements CarePlanRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<CarePlan>> {
    try {
  ...
  async get(orgId: OrgId, id: string): Promise<CarePlan | null> {
  ...
  async create(orgId: OrgId, data: CarePlan): Promise<CarePlan> {
    ...
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const item of items) {
          const record = await tx.carePlan.create({
            data: {
              ...item,
              orgId,
            },
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: CarePlanUpdate }>
  ): Promise<CarePlan[]> {
    try {
  ...
  private toDomain(model: any): CarePlan {
    return {
      ...model,
