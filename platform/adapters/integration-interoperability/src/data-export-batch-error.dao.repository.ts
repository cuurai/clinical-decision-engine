/**
 * DAO adapter for DataExportBatchErrorRepository
 *
...
export class DaoDataExportBatchErrorRepository implements DataExportBatchErrorRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.dataExportBatchError.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<DataExportBatchError | null> {
    try {
      const record = await this.dao.dataExportBatchError.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<DataExportBatchError | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("DataExportBatchError", id);
    }
    return result;
  }
  private toDomain(model: any): DataExportBatchError {
    return {
      ...model,
