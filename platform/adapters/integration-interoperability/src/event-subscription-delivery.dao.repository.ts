/**
 * DAO adapter for EventSubscriptionDeliveryRepository
 *
...
export class DaoEventSubscriptionDeliveryRepository implements EventSubscriptionDeliveryRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.eventSubscriptionDelivery.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<EventSubscriptionDelivery | null> {
    try {
      const record = await this.dao.eventSubscriptionDelivery.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<EventSubscriptionDelivery | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("EventSubscriptionDelivery", id);
    }
    return result;
  }
  private toDomain(model: any): EventSubscriptionDelivery {
    return {
      ...model,
