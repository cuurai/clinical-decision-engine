/**
 * DAO adapter for IntegrationRunLogRepository
 *
...
export class DaoIntegrationRunLogRepository implements IntegrationRunLogRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.integrationRunLog.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<IntegrationRunLog | null> {
    try {
      const record = await this.dao.integrationRunLog.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<IntegrationRunLog | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("IntegrationRunLog", id);
    }
    return result;
  }
  private toDomain(model: any): IntegrationRunLog {
    return {
      ...model,
