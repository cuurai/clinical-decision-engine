/**
 * DAO adapter for HL7MappingProfileRuleRepository
 *
...
export class DaoHL7MappingProfileRuleRepository implements HL7MappingProfileRuleRepository {
  ...
  ): Promise<PaginatedResult<HL7MappingProfileRule>> {
    try {
      const limit = params?.limit ?? DEFAULT_LIMIT;

      const records = await this.dao.hl7mappingProfileRule.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<HL7MappingProfileRule | null> {
    try {
      const record = await this.dao.hl7mappingProfileRule.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<HL7MappingProfileRule | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("HL7MappingProfileRule", id);
    }
    return result;
  }
  private toDomain(model: any): HL7MappingProfileRule {
    return {
      ...model,
