...
import type { UpdateExternalSystemRequest } from "@cuur-cde/core/integration-interoperability";
import type {
  ExternalSystemInput,
...
export class DaoExternalSystemRepository implements ExternalSystemRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<ExternalSystem>> {
    try {
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const item of items) {
          const record = await txClient.externalSystemInput.create({
            data: {
              ...item,
              orgId,
            },
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: ExternalSystemUpdate }>
  ): Promise<ExternalSystem[]> {
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await txClient.externalSystemInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
