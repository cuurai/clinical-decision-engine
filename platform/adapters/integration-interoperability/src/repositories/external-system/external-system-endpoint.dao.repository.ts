/**
 * DAO adapter for ExternalSystemEndpointRepository
 *
...
export class DaoExternalSystemEndpointRepository implements ExternalSystemEndpointRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.externalSystemEndpoint.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ExternalSystemEndpoint | null> {
    try {
      const record = await this.dao.externalSystemEndpoint.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ExternalSystemEndpoint | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ExternalSystemEndpoint", id);
    }
    return result;
  }
  private toDomain(model: any): ExternalSystemEndpoint {
    return {
      ...model,
