/**
 * DAO adapter for ExternalSystemConnectionRepository
 *
...
export class DaoExternalSystemConnectionRepository implements ExternalSystemConnectionRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.externalSystemConnection.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ExternalSystemConnection | null> {
    try {
      const record = await this.dao.externalSystemConnection.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ExternalSystemConnection | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ExternalSystemConnection", id);
    }
    return result;
  }
  private toDomain(model: any): ExternalSystemConnection {
    return {
      ...model,
