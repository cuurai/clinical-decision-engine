/**
 * DAO adapter for FHIRMappingProfileRuleRepository
 *
...
export class DaoFHIRMappingProfileRuleRepository implements FHIRMappingProfileRuleRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.fhirmappingProfileRule.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<FHIRMappingProfileRule | null> {
    try {
      const record = await this.dao.fhirmappingProfileRule.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<FHIRMappingProfileRule | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("FHIRMappingProfileRule", id);
    }
    return result;
  }
  private toDomain(model: any): FHIRMappingProfileRule {
    return {
      ...model,
