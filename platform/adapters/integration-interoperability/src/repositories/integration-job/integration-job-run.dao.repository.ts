/**
 * DAO adapter for IntegrationJobRunRepository
 *
...
export class DaoIntegrationJobRunRepository implements IntegrationJobRunRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.integrationJobRun.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<IntegrationJobRun | null> {
    try {
      const record = await this.dao.integrationJobRun.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<IntegrationJobRun | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("IntegrationJobRun", id);
    }
    return result;
  }
  private toDomain(model: any): IntegrationJobRun {
    return {
      ...model,
