/**
 * DAO adapter for ConnectionIntegrationJobRepository
 *
 * Generated by Adapter Generator v1.0.0
 * Generator Version: 1.0.0
 * Domain: integration-interoperability
 *
 * ⚠️  DO NOT EDIT THIS FILE MANUALLY
 * This file is auto-generated. Any manual changes will be overwritten.
 */

import type {
  OrgId,
  PaginatedResult,
  PaginationParams,
} from "@cuur-cde/core/_shared";
import type {
  ConnectionIntegrationJobRepository,
} from "@cuur-cde/core/integration-interoperability";
import type {
  ConnectionIntegrationJob,
} from "@cuur-cde/core/integration-interoperability";
import type { DaoClient } from "../db/dao-client.js";
import { NotFoundError, TransactionManager, handleDatabaseError } from "../utils/repository-helpers.js";

const DEFAULT_LIMIT = 50;

export class DaoConnectionIntegrationJobRepository implements ConnectionIntegrationJobRepository {
  private transactionManager: TransactionManager;

  constructor(private readonly dao: DaoClient) {
    this.transactionManager = new TransactionManager(dao);
  }

  async list(
    orgId: OrgId,
    params?: PaginationParams
  ): Promise<PaginatedResult<ConnectionIntegrationJob>> {
    try {
      const limit = params?.limit ?? DEFAULT_LIMIT;

      const records = await this.dao.connectionIntegrationJob.findMany({
        where: {
          orgId,
          deletedAt: null, // Soft delete filter - only return non-deleted records
        },
        orderBy: { createdAt: "desc" },
        take: limit,
        ...(params && 'cursor' in params && params.cursor ? {
          skip: 1,
          cursor: { id: params.cursor },
        } : {}),
      });

      return {
        items: records.map((r: any) => this.toDomain(r)),
        nextCursor: records.length === limit
          ? records[records.length - 1]?.id
          : undefined,
        prevCursor: undefined,
      };
    } catch (error) {
      handleDatabaseError(error);
      throw error;
    }
  }
  async findById(orgId: OrgId, id: string): Promise<ConnectionIntegrationJob | null> {
    try {
      const record = await this.dao.connectionIntegrationJob.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
        },
      });
      return record ? this.toDomain(record) : null;
    } catch (error) {
      handleDatabaseError(error);
      throw error;
    }
  }
  async get(orgId: OrgId, id: string): Promise<ConnectionIntegrationJob | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ConnectionIntegrationJob", id);
    }
    return result;
  }
  private toDomain(model: any): ConnectionIntegrationJob {
    return {
      ...model,
      createdAt: model.createdAt instanceof Date
        ? model.createdAt
        : new Date(model.createdAt),
      updatedAt: model.updatedAt instanceof Date
        ? model.updatedAt
        : model.updatedAt ? new Date(model.updatedAt) : undefined,
    } as ConnectionIntegrationJob;
  }
}
