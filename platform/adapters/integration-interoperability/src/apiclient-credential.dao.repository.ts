/**
 * DAO adapter for APIClientCredentialRepository
 *
...
export class DaoAPIClientCredentialRepository implements APIClientCredentialRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.apiclientCredential.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<APIClientCredential | null> {
    try {
      const record = await this.dao.apiclientCredential.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<APIClientCredential | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("APIClientCredential", id);
    }
    return result;
  }
  private toDomain(model: any): APIClientCredential {
    return {
      ...model,
