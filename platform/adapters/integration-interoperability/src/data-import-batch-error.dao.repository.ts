/**
 * DAO adapter for DataImportBatchErrorRepository
 *
...
export class DaoDataImportBatchErrorRepository implements DataImportBatchErrorRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.dataImportBatchError.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<DataImportBatchError | null> {
    try {
      const record = await this.dao.dataImportBatchError.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<DataImportBatchError | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("DataImportBatchError", id);
    }
    return result;
  }
  private toDomain(model: any): DataImportBatchError {
    return {
      ...model,
