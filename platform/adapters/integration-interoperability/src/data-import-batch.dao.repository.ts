...
import type { DataImportBatchRepository } from "@cuur-cde/core/integration-interoperability/repositories/index.js";
import type { UpdateDataImportBatchRequest } from "@cuur-cde/core/integration-interoperability/types/index.js";
import type {
...
export class DaoDataImportBatchRepository implements DataImportBatchRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<DataImportBatch>> {
    try {
      ...
      const records = await this.dao.dataImportBatchInput.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<DataImportBatch | null> {
    try {
      const record = await this.dao.dataImportBatchInput.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async create(orgId: OrgId, data: DataImportBatch): Promise<DataImportBatch> {
    ...
    try {
      const record = await this.dao.dataImportBatchInput.create({
        data: {
          ...data,
  ...
  async update(
    orgId: OrgId,
    id: string,
    data: UpdateDataImportBatchRequest
  ): Promise<DataImportBatch> {
    try {
      const record = await this.dao.dataImportBatchInput.update({
        where: { id, orgId },
        data: {
          ...data,
        },
  ...
  async delete(orgId: OrgId, id: string): Promise<void> {
    try {
      ...
      await this.dao.dataImportBatchInput.update({
        where: { id, orgId },
        data: {
          deletedAt: new Date(),
        },
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: DataImportBatchUpdate }>
  ): Promise<DataImportBatch[]> {
    try {
  ...
  async deleteMany(orgId: OrgId, ids: string[]): Promise<void> {
    try {
      ...
      await this.dao.dataImportBatchInput.updateMany({
        where: {
          id: { in: ids },
          orgId,
        },
