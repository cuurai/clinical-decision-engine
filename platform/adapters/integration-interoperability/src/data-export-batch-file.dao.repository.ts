/**
 * DAO adapter for DataExportBatchFileRepository
 *
...
export class DaoDataExportBatchFileRepository implements DataExportBatchFileRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.dataExportBatchFile.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<DataExportBatchFile | null> {
    try {
      const record = await this.dao.dataExportBatchFile.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<DataExportBatchFile | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("DataExportBatchFile", id);
    }
    return result;
  }
  private toDomain(model: any): DataExportBatchFile {
    return {
      ...model,
