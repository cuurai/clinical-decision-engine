/**
 * DAO adapter for RiskAssessmentRepository
 *
...
import type { RiskAssessmentRepository } from "@cuur-cde/core/decision-intelligence/repositories/index.js";
import type {
  RiskAssessmentInput,
...
export class DaoRiskAssessmentRepository implements RiskAssessmentRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<RiskAssessment>> {
    try {
  ...
  async get(orgId: OrgId, id: string): Promise<RiskAssessment | null> {
  ...
  async create(orgId: OrgId, data: RiskAssessment): Promise<RiskAssessment> {
    // Note: Repository interface expects RiskAssessment, but we only use input fields
  ...
  async updateMany(
    orgId: OrgId,
    updates: Array<{ id: string; data: UpdateRiskAssessmentRequest }>
  ): Promise<RiskAssessment[]> {
    try {
  ...
  private toDomain(model: any): RiskAssessment {
    return {
      ...model,
