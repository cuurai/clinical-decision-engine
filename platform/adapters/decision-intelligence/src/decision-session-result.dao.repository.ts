/**
 * DAO adapter for DecisionSessionResultRepository
 *
...
import type { DecisionSessionResultRepository } from "@cuur-cde/core/decision-intelligence/repositories/index.js";
import type { DecisionSessionResult } from "@cuur-cde/core/decision-intelligence/types/index.js";
...
export class DaoDecisionSessionResultRepository implements DecisionSessionResultRepository {
  ...
  async findById(orgId: OrgId, id: string): Promise<DecisionSessionResult | null> {
    try {
  ...
  async get(orgId: OrgId, id: string): Promise<DecisionSessionResult | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("DecisionSessionResult", id);
    }
    return result;
  }
  private toDomain(model: any): DecisionSessionResult {
    return {
      ...model,
