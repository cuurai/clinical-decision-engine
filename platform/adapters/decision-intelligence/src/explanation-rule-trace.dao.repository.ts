/**
 * DAO adapter for ExplanationRuleTraceRepository
 *
...
import type { ExplanationRuleTraceRepository } from "@cuur-cde/core/decision-intelligence/repositories/index.js";
import type { ExplanationRuleTrace } from "@cuur-cde/core/decision-intelligence/types/index.js";
...
export class DaoExplanationRuleTraceRepository implements ExplanationRuleTraceRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.explanationRuleTrace.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ExplanationRuleTrace | null> {
    try {
      const record = await this.dao.explanationRuleTrace.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ExplanationRuleTrace | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ExplanationRuleTrace", id);
    }
    return result;
  }
  private toDomain(model: any): ExplanationRuleTrace {
    return {
      ...model,
