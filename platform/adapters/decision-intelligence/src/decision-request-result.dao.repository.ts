/**
 * DAO adapter for DecisionRequestResultRepository
 *
...
import type { DecisionRequestResultRepository } from "@cuur-cde/core/decision-intelligence/repositories/index.js";
import type { DecisionRequestResult } from "@cuur-cde/core/decision-intelligence/types/index.js";
...
export class DaoDecisionRequestResultRepository implements DecisionRequestResultRepository {
  ...
  async findById(orgId: OrgId, id: string): Promise<DecisionRequestResult | null> {
    try {
  ...
  async get(orgId: OrgId, id: string): Promise<DecisionRequestResult | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("DecisionRequestResult", id);
    }
    return result;
  }
  private toDomain(model: any): DecisionRequestResult {
    return {
      ...model,
