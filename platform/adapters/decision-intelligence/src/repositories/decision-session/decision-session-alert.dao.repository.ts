/**
 * DAO adapter for DecisionSessionAlertRepository
 *
...
export class DaoDecisionSessionAlertRepository implements DecisionSessionAlertRepository {
  ...
  async get(orgId: OrgId, id: string): Promise<DecisionSessionAlert | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("DecisionSessionAlert", id);
    }
    return result;
  }
