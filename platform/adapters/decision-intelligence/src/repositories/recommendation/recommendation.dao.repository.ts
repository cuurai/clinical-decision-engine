...
export class DaoRecommendationRepository implements RecommendationRepository {
  ...
  async get(orgId: OrgId, id: string): Promise<Recommendation | null> {
  ...
  async create(orgId: OrgId, data: Recommendation): Promise<Recommendation> {
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const item of items) {
          const record = await txClient.recommendationInput.create({
            data: {
              ...item,
              orgId,
            },
    ...
    try {
      ...
      return await this.tx.run(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await txClient.recommendation.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
