/**
 * DAO adapter for SimulationRunDecisionResultRepository
 *
...
import type { SimulationRunDecisionResultRepository } from "@cuur-cde/core/decision-intelligence/repositories/index.js";
import type { SimulationRunDecisionResult } from "@cuur-cde/core/decision-intelligence/types/index.js";
  ...
  implements SimulationRunDecisionResultRepository
{
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.simulationRunDecisionResult.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<SimulationRunDecisionResult | null> {
    try {
      const record = await this.dao.simulationRunDecisionResult.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<SimulationRunDecisionResult | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("SimulationRunDecisionResult", id);
    }
    return result;
  }
  private toDomain(model: any): SimulationRunDecisionResult {
    return {
      ...model,
      ...
      updatedAt:
        model.updatedAt instanceof Date
          ...
          : undefined,
    } as SimulationRunDecisionResult;
  }
}
