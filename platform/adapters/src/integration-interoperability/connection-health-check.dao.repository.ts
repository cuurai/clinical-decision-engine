/**
 * DAO adapter for ConnectionHealthCheckRepository
 *
...
export class DaoConnectionHealthCheckRepository implements ConnectionHealthCheckRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.connectionHealthCheck.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ConnectionHealthCheck | null> {
    try {
      const record = await this.dao.connectionHealthCheck.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ConnectionHealthCheck | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ConnectionHealthCheck", id);
    }
    return result;
  }
  private toDomain(model: any): ConnectionHealthCheck {
    return {
      ...model,
