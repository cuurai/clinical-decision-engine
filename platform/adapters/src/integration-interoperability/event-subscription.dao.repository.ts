...
export class DaoEventSubscriptionRepository implements EventSubscriptionRepository {
  ...
  async create(orgId: OrgId, data: EventSubscriptionInput, createdBy?: string): Promise<Timestamps> {
    try {
  ...
  async update(orgId: OrgId, id: string, data: EventSubscriptionUpdate, updatedBy?: string): Promise<Timestamps> {
    try {
  ...
  async createMany(orgId: OrgId, items: Array<EventSubscriptionInput>): Promise<Timestamps[]> {
    try {
      // Use createMany for better performance
      await this.dao.eventSubscription.createMany({
        data: items.map(item => ({
          ...item,
          orgId,
        })),
  ...
  async updateMany(orgId: OrgId, updates: Array<{ id: string; data: EventSubscriptionUpdate }>): Promise<Timestamps[]> {
    try {
