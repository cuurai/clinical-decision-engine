/**
 * DAO adapter for EpisodesOfCareRepository
 *
...
export class DaoEpisodesOfCareRepository implements EpisodesOfCareRepository {
  ...
  async findById(orgId: OrgId, id: string): Promise<EpisodesOfCare | null> {
    try {
  ...
  async get(orgId: OrgId, id: string): Promise<EpisodesOfCare | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("EpisodesOfCare", id);
    }
    return result;
  }
  private toDomain(model: any): EpisodesOfCare {
    return {
      ...model,
