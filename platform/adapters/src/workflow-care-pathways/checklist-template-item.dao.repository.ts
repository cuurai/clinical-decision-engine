/**
 * DAO adapter for ChecklistTemplateItemRepository
 *
...
export class DaoChecklistTemplateItemRepository implements ChecklistTemplateItemRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.checklistTemplateItem.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ChecklistTemplateItem | null> {
    try {
      const record = await this.dao.checklistTemplateItem.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ChecklistTemplateItem | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ChecklistTemplateItem", id);
    }
    return result;
  }
  private toDomain(model: any): ChecklistTemplateItem {
    return {
      ...model,
