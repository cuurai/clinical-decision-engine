/**
 * DAO adapter for TaskAuditEventRepository
 *
...
export class DaoTaskAuditEventRepository implements TaskAuditEventRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.taskAuditEvent.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<TaskAuditEvent | null> {
    try {
      const record = await this.dao.taskAuditEvent.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<TaskAuditEvent | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("TaskAuditEvent", id);
    }
    return result;
  }
  private toDomain(model: any): TaskAuditEvent {
    return {
      ...model,
