...
export class DaoValueSetRepository implements ValueSetRepository {
  ...
  async create(orgId: OrgId, data: ValueSetInput, createdBy?: string): Promise<Timestamps> {
    try {
  ...
  async update(orgId: OrgId, id: string, data: ValueSetUpdate, updatedBy?: string): Promise<Timestamps> {
    try {
  ...
  async createMany(orgId: OrgId, items: Array<ValueSetInput>): Promise<Timestamps[]> {
    try {
      // Use createMany for better performance
      await this.dao.valueSet.createMany({
        data: items.map(item => ({
          ...item,
          orgId,
        })),
  ...
  async updateMany(orgId: OrgId, updates: Array<{ id: string; data: ValueSetUpdate }>): Promise<Timestamps[]> {
    try {
