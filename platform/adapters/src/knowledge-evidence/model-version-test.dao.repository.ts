/**
 * DAO adapter for ModelVersionTestRepository
 *
...
export class DaoModelVersionTestRepository implements ModelVersionTestRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.modelVersionTest.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ModelVersionTest | null> {
    try {
      const record = await this.dao.modelVersionTest.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ModelVersionTest | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ModelVersionTest", id);
    }
    return result;
  }
  private toDomain(model: any): ModelVersionTest {
    return {
      ...model,
