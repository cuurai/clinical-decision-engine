/**
 * DAO adapter for PatientCareTeamRepository
 *
...
export class DaoPatientCareTeamRepository implements PatientCareTeamRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientCareTeam.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientCareTeam | null> {
    try {
      const record = await this.dao.patientCareTeam.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientCareTeam | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientCareTeam", id);
    }
    return result;
  }
  private toDomain(model: any): PatientCareTeam {
    return {
      ...model,
