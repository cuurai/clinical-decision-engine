/**
 * DAO adapter for EncounterDiagnosticReportRepository
 *
...
export class DaoEncounterDiagnosticReportRepository implements EncounterDiagnosticReportRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.encounterDiagnosticReport.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<EncounterDiagnosticReport | null> {
    try {
      const record = await this.dao.encounterDiagnosticReport.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<EncounterDiagnosticReport | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("EncounterDiagnosticReport", id);
    }
    return result;
  }
  private toDomain(model: any): EncounterDiagnosticReport {
    return {
      ...model,
