/**
 * DAO adapter for PatientImmunizationRepository
 *
...
export class DaoPatientImmunizationRepository implements PatientImmunizationRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientImmunization.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientImmunization | null> {
    try {
      const record = await this.dao.patientImmunization.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientImmunization | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientImmunization", id);
    }
    return result;
  }
  private toDomain(model: any): PatientImmunization {
    return {
      ...model,
