/**
 * DAO adapter for EncounterNoteRepository
 *
...
export class DaoEncounterNoteRepository implements EncounterNoteRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.encounterNote.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<EncounterNote | null> {
    try {
      const record = await this.dao.encounterNote.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<EncounterNote | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("EncounterNote", id);
    }
    return result;
  }
  private toDomain(model: any): EncounterNote {
    return {
      ...model,
