/**
 * DAO adapter for PatientConditionRepository
 *
...
export class DaoPatientConditionRepository implements PatientConditionRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientCondition.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientCondition | null> {
    try {
      const record = await this.dao.patientCondition.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientCondition | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientCondition", id);
    }
    return result;
  }
  private toDomain(model: any): PatientCondition {
    return {
      ...model,
