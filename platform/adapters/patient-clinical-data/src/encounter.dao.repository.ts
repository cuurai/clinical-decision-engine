/**
 * DAO adapter for EncounterRepository
 *
...
import type { EncounterRepository } from "@cuur-cde/core/patient-clinical-data/repositories/index.js";
import type {
  EncounterInput,
  EncounterUpdate,
...
export class DaoEncounterRepository implements EncounterRepository {
  ...
  async list(orgId: OrgId, params?: PaginationParams): Promise<PaginatedResult<Encounter>> {
    try {
      ...
      const records = await this.dao.encounter.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<Encounter | null> {
    try {
      const record = await this.dao.encounter.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<Encounter | null> {
  ...
  async create(orgId: OrgId, data: Encounter): Promise<Encounter> {
  ...
  async update(orgId: OrgId, id: string, data: UpdateEncounterRequest): Promise<Encounter> {
    try {
  ...
  async createMany(orgId: OrgId, items: Array<EncounterInput>): Promise<Encounter[]> {
    try {
      // Use transaction with individual creates to get created records with IDs
      return await this.transactionManager.executeInTransaction(async (tx) => {
        const results: Encounter[] = [];
        for (const item of items) {
          const record = await tx.encounter.create({
            data: {
              ...item,
              orgId,
            },
  ...
  async deleteMany(orgId: OrgId, ids: string[]): Promise<void> {
    try {
      ...
      await this.dao.encounter.updateMany({
        where: {
          id: { in: ids },
          orgId,
        },
