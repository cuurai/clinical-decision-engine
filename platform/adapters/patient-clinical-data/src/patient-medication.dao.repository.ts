/**
 * DAO adapter for PatientMedicationRepository
 *
...
export class DaoPatientMedicationRepository implements PatientMedicationRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientMedication.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientMedication | null> {
    try {
      const record = await this.dao.patientMedication.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientMedication | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientMedication", id);
    }
    return result;
  }
  private toDomain(model: any): PatientMedication {
    return {
      ...model,
