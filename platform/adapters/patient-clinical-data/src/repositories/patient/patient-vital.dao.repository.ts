/**
 * DAO adapter for PatientVitalRepository
 *
...
export class DaoPatientVitalRepository implements PatientVitalRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientVital.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientVital | null> {
    try {
      const record = await this.dao.patientVital.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientVital | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientVital", id);
    }
    return result;
  }
  private toDomain(model: any): PatientVital {
    return {
      ...model,
