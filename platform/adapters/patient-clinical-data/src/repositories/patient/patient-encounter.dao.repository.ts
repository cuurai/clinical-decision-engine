/**
 * DAO adapter for PatientEncounterRepository
 *
...
export class DaoPatientEncounterRepository implements PatientEncounterRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientEncounter.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientEncounter | null> {
    try {
      const record = await this.dao.patientEncounter.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientEncounter | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientEncounter", id);
    }
    return result;
  }
  private toDomain(model: any): PatientEncounter {
    return {
      ...model,
