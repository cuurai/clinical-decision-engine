/**
 * DAO adapter for PatientNoteRepository
 *
...
export class DaoPatientNoteRepository implements PatientNoteRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientNote.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientNote | null> {
    try {
      const record = await this.dao.patientNote.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientNote | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientNote", id);
    }
    return result;
  }
  private toDomain(model: any): PatientNote {
    return {
      ...model,
