/**
 * DAO adapter for PatientSummaryRepository
 *
...
export class DaoPatientSummaryRepository implements PatientSummaryRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientSummary.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientSummary | null> {
    try {
      const record = await this.dao.patientSummary.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientSummary | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientSummary", id);
    }
    return result;
  }
  private toDomain(model: any): PatientSummary {
    return {
      ...model,
