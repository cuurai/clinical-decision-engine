/**
 * DAO adapter for DiagnosticReportImagingStudyRepository
 *
...
export class DaoDiagnosticReportImagingStudyRepository implements DiagnosticReportImagingStudyRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.diagnosticReportImagingStudy.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<DiagnosticReportImagingStudy | null> {
    try {
      const record = await this.dao.diagnosticReportImagingStudy.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<DiagnosticReportImagingStudy | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("DiagnosticReportImagingStudy", id);
    }
    return result;
  }
  private toDomain(model: any): DiagnosticReportImagingStudy {
    return {
      ...model,
    ...
    } as DiagnosticReportImagingStudy;
  }
}
