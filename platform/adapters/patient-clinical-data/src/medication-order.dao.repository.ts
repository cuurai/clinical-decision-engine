/**
 * DAO adapter for MedicationOrderRepository
 *
...
import type { MedicationOrderRepository } from "@cuur-cde/core/patient-clinical-data/repositories/index.js";
import type {
...
export class DaoMedicationOrderRepository implements MedicationOrderRepository {
  ...
  async get(orgId: OrgId, id: string): Promise<MedicationOrder | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("MedicationOrder", id);
    }
    return result;
  }
  async create(orgId: OrgId, data: MedicationOrder): Promise<MedicationOrder> {
    // Note: Repository interface expects MedicationOrder, but we only use input fields
  ...
  async createMany(orgId: OrgId, items: Array<MedicationOrderInput>): Promise<MedicationOrder[]> {
    try {
      // Use transaction with individual creates to get created records with IDs
      return await this.transactionManager.executeInTransaction(async (tx) => {
        const results: MedicationOrder[] = [];
        for (const item of items) {
          const record = await tx.medicationOrderInput.create({
            data: {
              ...item,
              orgId,
            },
    ...
    try {
      ...
      return await this.transactionManager.executeInTransaction(async (tx) => {
        ...
        for (const { id, data } of updates) {
          const record = await tx.medicationOrderInput.update({
            where: { id, orgId },
            data,
          });
          results.push(this.toDomain(record));
