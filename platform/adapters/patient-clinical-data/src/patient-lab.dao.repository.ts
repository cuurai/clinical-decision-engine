/**
 * DAO adapter for PatientLabRepository
 *
...
export class DaoPatientLabRepository implements PatientLabRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientLab.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientLab | null> {
    try {
      const record = await this.dao.patientLab.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientLab | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientLab", id);
    }
    return result;
  }
  private toDomain(model: any): PatientLab {
    return {
      ...model,
