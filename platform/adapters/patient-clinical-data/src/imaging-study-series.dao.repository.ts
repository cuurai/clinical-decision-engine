/**
 * DAO adapter for ImagingStudySeriesRepository
 *
...
export class DaoImagingStudySeriesRepository implements ImagingStudySeriesRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.imagingStudySeries.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ImagingStudySeries | null> {
    try {
      const record = await this.dao.imagingStudySeries.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ImagingStudySeries | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ImagingStudySeries", id);
    }
    return result;
  }
  private toDomain(model: any): ImagingStudySeries {
    return {
      ...model,
