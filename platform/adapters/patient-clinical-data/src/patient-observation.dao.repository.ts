/**
 * DAO adapter for PatientObservationRepository
 *
...
export class DaoPatientObservationRepository implements PatientObservationRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.patientObservation.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<PatientObservation | null> {
    try {
      const record = await this.dao.patientObservation.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<PatientObservation | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("PatientObservation", id);
    }
    return result;
  }
  private toDomain(model: any): PatientObservation {
    return {
      ...model,
