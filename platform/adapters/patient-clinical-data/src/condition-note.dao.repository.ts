/**
 * DAO adapter for ConditionNoteRepository
 *
...
export class DaoConditionNoteRepository implements ConditionNoteRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.conditionNote.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<ConditionNote | null> {
    try {
      const record = await this.dao.conditionNote.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<ConditionNote | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("ConditionNote", id);
    }
    return result;
  }
  private toDomain(model: any): ConditionNote {
    return {
      ...model,
