...
class FileGenerator(BaseGenerator):
    ...
    def get_layer(self) -> str:
        """
        Get the layer name for this generator.

        Override in subclass to specify the layer.

        Returns:
            Layer name (e.g., "services", "adapters", "tests")
        """
        # Default mapping based on generator type
        layer_map = {
            "service": "services",
            "adapter": "adapters",
            "tests": "tests",
        }
        return layer_map.get(self.type, "services")
