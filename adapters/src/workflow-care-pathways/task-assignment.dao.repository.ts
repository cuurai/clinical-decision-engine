...
export class DaoTaskAssignmentRepository implements TaskAssignmentRepository {
  ...
  async create(orgId: OrgId, data: TaskAssignmentInput, createdBy?: string): Promise<Timestamps> {
    try {
  ...
  async update(orgId: OrgId, id: string, data: TaskAssignmentUpdate, updatedBy?: string): Promise<Timestamps> {
    try {
  ...
  async createMany(orgId: OrgId, items: Array<TaskAssignmentInput>): Promise<Timestamps[]> {
    try {
      // Use createMany for better performance
      await this.dao.taskAssignment.createMany({
        data: items.map(item => ({
          ...item,
          orgId,
        })),
  ...
  async updateMany(orgId: OrgId, updates: Array<{ id: string; data: TaskAssignmentUpdate }>): Promise<Timestamps[]> {
    try {
