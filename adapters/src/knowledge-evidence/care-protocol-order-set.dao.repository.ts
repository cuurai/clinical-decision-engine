/**
 * DAO adapter for CareProtocolOrderSetRepository
 *
...
export class DaoCareProtocolOrderSetRepository implements CareProtocolOrderSetRepository {
  ...
  async list(
    ...
    try {
      ...
      const records = await this.dao.careProtocolOrderSet.findMany({
        where: {
          orgId,
  ...
  async findById(orgId: OrgId, id: string): Promise<CareProtocolOrderSet | null> {
    try {
      const record = await this.dao.careProtocolOrderSet.findFirst({
        where: {
          orgId,
          id,
          deletedAt: null, // Soft delete filter - only return non-deleted records
  ...
  async get(orgId: OrgId, id: string): Promise<CareProtocolOrderSet | null> {
    const result = await this.findById(orgId, id);
    if (!result) {
      throw new NotFoundError("CareProtocolOrderSet", id);
    }
    return result;
  }
  private toDomain(model: any): CareProtocolOrderSet {
    return {
      ...model,
