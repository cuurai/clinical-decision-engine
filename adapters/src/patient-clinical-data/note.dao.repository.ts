/**
 * DAO adapter for NoteRepository
 *
...
export class DaoNoteRepository implements NoteRepository {
  ...
  async create(orgId: OrgId, data: ClinicalNoteInput, createdBy?: string): Promise<Timestamps> {
    try {
  ...
  async update(orgId: OrgId, id: string, data: ClinicalNoteUpdate, updatedBy?: string): Promise<Timestamps> {
    try {
  ...
  async createMany(orgId: OrgId, items: Array<ClinicalNoteInput>): Promise<Timestamps[]> {
    try {
      // Use createMany for better performance
      await this.dao.note.createMany({
        data: items.map(item => ({
          ...item,
          orgId,
        })),
  ...
  async updateMany(orgId: OrgId, updates: Array<{ id: string; data: ClinicalNoteUpdate }>): Promise<Timestamps[]> {
    try {
